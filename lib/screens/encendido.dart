...
class _EncendidoState extends State<Encendido> {
  ...
  Future<void> _turnOnLights() async {
    try {
      await dbRef.set({
        "estado": "on",
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("✅ Luces encendidas correctamente"),
            duration: Duration(seconds: 2),
          ),
        );

        // Navegar después de confirmar que Firebase ha sido actualizado
        Navigator.push(
          context,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("❌ Error al encender las luces: $error"),
