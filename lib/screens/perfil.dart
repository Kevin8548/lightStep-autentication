...
class _PerfilState extends State<Perfil> {
  ...
  final TextEditingController _feedbackController =
      TextEditingController(); // Controlador para comentarios
  final String formspreeURL =
      "https://formspree.io/f/xpwpywyq"; // Reemplaza con tu URL de Formspree
  ...
  Future<void> _sendFeedback() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text("Error al enviar el comentario: ${response.body}")),
        );
      }
    } catch (error) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("Error de conexión al enviar el comentario")),
      );
    }
  }
